...
<div class="row wrapper border-bottom white-bg page-heading">
                <div class="col-lg-10">
                    ...
                    <ol class="breadcrumb">
                        ...
                        <li class="active">
                            <strong>Edit Purchase Items</strong>
                        </li>
                    </ol>
                </div>
                <div class="col-lg-2">
...
<div class="row">
                <div class="col-lg-12">
                    <div class="ibox float-e-margins">
						...
						<input type="text" class="form-control datepicker" id="name" value="{{date("d-m-Y" , strtotime($purchase->purchase_date))}}" name="purcahse_date" placeholder="Purchase Date" autocomplete="off" data-field_type="input">
						</div>
                        <div class="col-sm-4 col-lg-4 form-group">
							<label>Bill No</label>
							<input type="text" class="form-control" id="name" name="bill_no" placeholder="Bill No" value="{{$purchase->bill_no}}" autocomplete="off" data-field_type="input">
						</div>
						<div class="col-sm-4 col-lg-4 form-group">
							<label>Supplier</label>
							 <select class="form-control" id="supplier_id" name="supplier_id"> 
											@foreach($suppliers as $cat)
											<option @if($cat->supplier_id == $cat->id) selected @endif value="{{$cat->id}}">  {{$cat->company_name}} </option>
											@endforeach
									</select>
                        </div>
					...
					<?php $total_price = 0; $p_ids = []; ?>
                         ...
                         <tbody id="new_product">
								@foreach($purchase_items as $item)
									...
									<tr>
										<td>{{$product->name}}</td>
										<td><input type='text' value='{{$item->quantity}}' id='qty_{{$item->product_id}}' data-id='{{$item->product_id}}' name='quantity[]' class='form-control change_qty'></td>
										<td><input type='text' value='{{$item->unit_price}}' data-id='{{$item->product_id}}'  id='price_{{$item->product_id}}' name='purchase_cost[]' class='form-control change_price'> 
											<input type='hidden' name='product_ids[]' value='{{$item->product_id}}'> </td>
										<td><input type='text' value='{{$item->units}}' data-id='{{$item->product_id}}'  id='units_{{$item->product_id}}' name='units[]' class='form-control units'></td>
										<td>{{$item->units * $item->quantity}}</td>
										<td><input type='text' name='sold_price[]' value="{{ $item->sold_price}}" class='form-control'></td>
										<td><input type='text' value='{{$item->quantity * $item->unit_price}}' readonly id='toatlprice_{{$item->product_id}}' name='total_price[]' class='form-control'></td>
										
								</tr>
								<?php $total_price += $item->quantity * $item->unit_price;  $p_ids[] = $item->product_id;?>
								@endif
								@endforeach
                         </tbody>
                    ...
                    <thead id="sale_footer" >
								...
								<?php $total_price += $purchase->tax - $purchase->discount;  ?>
								
								<tr>
									<th colspan=6> Tax </th>
									<th> <input type="text" value="0" min=0 name="tax" value="{{$purchase->tax}}" class="form-control" id="tax" style="" /> </th>
								</tr>
								
								<tr>
									
									<th colspan=6> Total Discount </th>
									<th> <input type="text" value="0" min=0 name="discount" value="{{$purchase->discount}}" class="form-control" id="discount" style="" /> </th>
								</tr>
